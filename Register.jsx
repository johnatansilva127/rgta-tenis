...
import { supabase, initials, compatible, calcPoints } from './supabaseClient'

const today = () => new Date().toISOString().slice(0, 10)

export default function Register({ session, profile, nav }) {
  ...
  async function save() {
    ...
    const { error } = await supabase.rpc('register_match', {
      ...
      p_result: result,
      p_set_scores: scores,
      p_went_super: superTb,
      p_played_at: date,
    })
    setSaving(false)
    if (error) return setErr(error.message)
    setOk('Partida enviada! Aguardando aprovação do administrador.')
    ...
    <>
      ...
      <div className="scroll"><div className="sec">
        ...
        <div className="vs-box">
          <div className="sel"><div className="ava">{profile ? initials(profile.name) : '·'}</div> Você ({myCat})</div>
        ...
        {players.length === 0 && <div style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 6 }}>Nenhum adversário compatível cadastrado ainda. (A não joga contra C.)</div>}
        ...
        {preview && (
          <div className="ptsbox">
            Pontos (após aprovação): <b>você +{preview.me}</b> · adversário +{preview.opp}
          </div>
        )}
